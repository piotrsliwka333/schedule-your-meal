	...
	return(
		<Container fluid className='pr4 pl-4 h-100'>
			...
			<ApplicationListElements>
				{recipes.map(element => <ApplicationListElement editElement={editExistingSchedule} userUid={userUid}  elementToEditOrDelete={element.id} deleteElement={handleDeleteElement} key={element.id} name={element.title} description={element.description}/> )}
			</ApplicationListElements>
		</Container>
	)
}