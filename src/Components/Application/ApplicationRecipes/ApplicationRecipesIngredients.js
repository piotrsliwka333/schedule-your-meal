...
export const ApplicationRecipesIngredients = (props) => {
	...
	const handleCheckAddIngredient = (e, newIngredientValue, newRecipesIngredients) => {
	...
	return (
		<Col className='recipes__ingredients col-12 col-md-6 col-xl-6'>
			...
			<form onSubmit={e => handleCheckAddIngredient(e, newIngredientValue, newRecipe.ingredients)}
			      className='ingredients-form'>
				<div className='ingredients-form-wrapper'>
					<input onChange={e => handleCheckIngredientValidation(e)} value={newIngredientValue}
					       placeholder='podaj nazwę składnika'
					       className={newIngredientError ? 'ingredients-form__input error' : 'ingredients-form__input'}/>
					<button type='submit' className='ingredients-form__button'>
						<i className="fas fa-plus-square"/>
					</button>
					{newIngredientError &&
					<p
						className='error-message'>{newIngredientValue.length > 150 ? 'max 150 znaków' : 'pole nie może być puste'}</p>}
				</div>
			</form>
