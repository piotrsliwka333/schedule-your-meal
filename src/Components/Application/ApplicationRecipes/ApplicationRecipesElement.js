...
export const ApplicationRecipesElement = (props) => {
	...
	return (
		<li className='recipes-element'>
			<input onChange={e => handleEditInputValue(e)} style={style} disabled={disabled} value={startInputValue}
			       className='recipes-element__text'/>
			<div className='recipes-element__icon-box'>
				<button onClick={e => handleEdit(e)} className='recipes-element__edit-button '>
					<i className="far fa-edit"/>
				</button>
				<button onClick={e => handleCheckDeleteElement(e, array, elementToEditOrDelete, arrayToEdit)}
				        className='recipes-element__delete-button '>
					<i className="far fa-trash-alt"/>
				</button>
				{!disabled && <button name={name}
				                      onClick={e => handleCheckSaveEditedElement(e, array, elementToEditOrDelete, startInputValue, arrayToEdit)}
				                      className='recipes-element__save-button'><i className="fas fa-save"/></button>}
			</div>
		</li>
	)
}